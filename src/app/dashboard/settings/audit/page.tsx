    ...
    <div className="flex flex-col gap-5">
      <section className="rounded-xl border border-border bg-card p-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold">Activiteitenlogboek</h2>
        </div>
        <p className="mt-1 text-xs text-muted-foreground">
          Alle wijzigingen binnen <strong>{ctx.organization.name}</strong> — voor
          inzicht en accountability tussen teamleden.
        </p>
      </section>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex gap-2">
            {pageNum > 1 && (
              <Link
                href={`/dashboard/settings/audit?page=${pageNum - 1}`}
