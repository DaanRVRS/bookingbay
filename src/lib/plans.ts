...
export type PlanFeature = keyof Omit<PlanLimits, "maxItems" | "maxMembers" | "monthlyPriceEuro" | "label">;
...
export function describeLimit(plan: Plan, kind: "items" | "members"): string {
  const v = kind === "items" ? PLAN_LIMITS[plan].maxItems : PLAN_LIMITS[plan].maxMembers;
